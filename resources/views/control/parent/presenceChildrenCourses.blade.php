    ...
    <table class="table" style="overflow: hidden" id="funSearch">
        ...
        <tbody>
            ...
            <tr >
                <td>{{$row->name}}</td>
                <td>
                    @foreach ($findPresenceChildrencourses as $item)
                        @if ($item->student_id == $row->id)
                            <p data-toggle="tooltip" data-placement="right" title="{{App\courses::find($item->course_id)->title}}"  class="@if($item->status == 1)  text-success  @else text-warning  @endif"> {{$item->date}}  </p>
                        @endif
                    @endforeach
                </td>
